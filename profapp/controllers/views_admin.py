...
from ..models.permissions import UserIsAdminOrMaintainer, AvailableForAll


@admin_bp.route('/', methods=['GET'], permissions=AvailableForAll())
def index():
    return render_template('admin/index.html')


@admin_bp.route('/translations', methods=['GET'], permissions=UserIsAdminOrMaintainer())
def translations():
...
@admin_bp.route('/translations', methods=['OK'], permissions=UserIsAdminOrMaintainer())
...
@admin_bp.route('/translations_save', methods=['OK'], permissions=UserIsAdminOrMaintainer())
...
@admin_bp.route('/delete', methods=['OK'], permissions=UserIsAdminOrMaintainer())
...
@admin_bp.route('/ips', methods=['GET'], permissions=UserIsAdminOrMaintainer())
def ips():
    return render_template('admin/ips.html',
                           angular_ui_bootstrap_version='//angular-ui.github.io/bootstrap/ui-bootstrap-tpls-0.14.2.js')


@admin_bp.route('/ips', methods=['OK'], permissions=UserIsAdminOrMaintainer())
def ips_load(json):
    ...
    if json.get('editItem'):
        exist = utils.db.query_filter(Ips, template=json.get('editItem')['template'],
                                      name=json.get('editItem')['name']).first()
        i = datetime.datetime.now()
        Ips.get(exist.id).attr(
            {json.get('editItem')['col']: json.get('editItem')['newValue'], 'md_tm': i}).save().get_client_side_dict()
...
@admin_bp.route('/ips_save', methods=['OK'], permissions=UserIsAdminOrMaintainer())
def ips_save(json):
...
@admin_bp.route('/ips_add', methods=['OK'], permissions=UserIsAdminOrMaintainer())
def ips_add(json):
    exist = utils.db.query_filter(Ips, template=json['row'], name=json['col']).first()
    return Ips.get(exist.id).attr({json['lang']: json['val']}).add().get_client_side_dict()


@admin_bp.route('/ips_delete', methods=['OK'], permissions=UserIsAdminOrMaintainer())
def ips_delete(json):
    return Ips.delete(json['objects'])


@admin_bp.route('/config', methods=['GET'], permissions=UserIsAdminOrMaintainer())
def config():
    return render_template('admin/config.html',
                           angular_ui_bootstrap_version='//angular-ui.github.io/bootstrap/ui-bootstrap-tpls-0.14.2.js')


@admin_bp.route('/config', methods=['OK'], permissions=UserIsAdminOrMaintainer())
def config_load(json):
    ...
    if json.get('search_text'):
        for d in json.get('search_text'):
            params['search_text'][d] = json.get('search_text')[d]
    if json.get('editItem'):
        exist = utils.db.query_filter(Config, template=json.get('editItem')['template'],
                                      name=json.get('editItem')['name']).first()
        i = datetime.datetime.now()
